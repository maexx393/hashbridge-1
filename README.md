...
<a href="https://github.com/HashProtocol/HashBridge/actions">
  <img alt="build status" src="https://github.com/HashProtocol/HashBridge/workflows/Tests/badge.svg?branch=master" />
</a>
...
# Getting Started
- Check out our [documentation](https://hashbridge.HashProtocol.io).
- Try [running HashBridge locally](https://hashbridge.HashProtocol.io/local/).
...
`make build`: Builds `hashbridge` in `./build`.

**or**

`make install`: Uses `go install` to add `hashbridge` to your GOBIN.

## Docker 
The official HashBridge Docker image can be found here.

To build the Docker image locally run:

```
docker build -t hashprotocol/hashbridge .
```
...
``` 
docker run -v $(pwd)/config.json:/config.json hashprotocol/hashbridge
```
...
HashBridge requires keys to sign and submit transactions, and to identify each bridge node on chain.

To use secure keys, see `hashbridge accounts --help`. The keystore password can be supplied with the `KEYSTORE_PASSWORD` environment variable.

To import external ethereum keys, such as those generated with geth, use `hashbridge accounts import --ethereum /path/to/key`.

To import private keys as keystores, use `hashbridge account import --privateKey key`.

For testing purposes, hashbridge provides 5 test keys. The can be used with `--testkey <name>`, where `name` is one of `Alice`, `Bob`, `Charlie`, `Dave`, or `Eve`. 
...
- Ethereum (Solidity): [hashbridge-solidity](https://github.com/HashProtocol/hashbridge-solidity) 

    The Solidity contracts required for hashbridge. Includes deployment and interaction CLI.
...
- Substrate: [hashbridge-substrate](https://github.com/HashProtocol/hashbridge-substrate)

    A substrate pallet that can be integrated into a chain, as well as an example pallet to demonstrate chain integration.

# Docs

MKdocs will generate static HTML files for HashProtocol markdown files located in `Hashbridge/docs/`
...
[security at hashprotocol dot io](mailto:security@hashprotocol.io).
