...
build:
	...
	cd cmd/hashbridge && env GOARCH=amd64 go build -o ../../build/hashbridge $(VERSION)

install:
	@echo "  >  \033[32mInstalling bridge...\033[0m "
	cd cmd/hashbridge && go install $(VERSION)
...
license:
	...
	addlicense -c "hashprotocol " -f ./scripts/header.txt -y 2020 .

## license-check: Checks for missing license headers
license-check:
	@echo "  >  \033[Checking for license headers...\033[0m "
	GO111MODULE=off go get -u github.com/google/addlicense
	addlicense -check -c "hashprotocol " -f ./scripts/header.txt -y 2020 .
