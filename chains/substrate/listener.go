...
import (
	...
	utils "github.com/hashprotocol/hashbridge/shared/substrate"
...
func (l *listener) handleEvents(evts utils.Events) {
	if l.subscriptions[FungibleTransfer] != nil {
		for _, evt := range evts.HashBridge_FungibleTransfer {
	...
	if l.subscriptions[NonFungibleTransfer] != nil {
		for _, evt := range evts.HashBridge_NonFungibleTransfer {
	...
	if l.subscriptions[GenericTransfer] != nil {
		for _, evt := range evts.HashBridge_GenericTransfer {
